...
const GuidanceDashboard = () => {
  ...
  useEffect(() => {
    let mounted = true;
    api.get('/api/v1/guidance/dashboard')
      .then(res => {
        if (!mounted) return;
  ...
  if (forbidden) return (
    <div className="p-8">
      ...
      <p className="text-gray-600">You do not have permission to view the guidance dashboard.</p>
    </div>
  );

  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-4">Guidance Dashboard</h1>
      {user ? (
        <div className="bg-white dark:bg-gray-800 p-6 rounded shadow">
          <p className="text-lg">Hello, <strong>{user.name}</strong></p>
          <p className="text-sm text-gray-600 dark:text-gray-300">Role: {user.role ?? 'guidance'}</p>
          <div className="mt-4">
            <p className="text-sm">This is a placeholder dashboard for guidance counselors. Add appointment management and student lists here.</p>
          </div>
        </div>
      ) : (
